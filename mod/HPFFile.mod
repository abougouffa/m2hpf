...
BEGIN
  ...
  RawRead(hpfFile.chanId, ADR(hpfFile.channelInformation), 24, readCount);
  xmlBufferSize := hpfFile.channelInformation.chunkSize - 24;
  ALLOCATE(ptrXmlBuffer:ADDRESS, xmlBufferSize);
  RawRead(hpfFile.chanId, ptrXmlBuffer:ADDRESS, xmlBufferSize, readCount);
...
BEGIN
  ...
  LOOP
    RawRead(hpfFile.chanId, ADR(readChunk), 16, readCount); (* read chunkId and chunkSize *)
...
BEGIN
  ...
  RawRead(hpfFile.chanId, ADR(hpfFile.index), 24, readCount);

  (* Allocate and read the events list *)
  NEW(hpfFile.index.event, hpfFile.index.indexCount - 1);
  RawRead(hpfFile.chanId, hpfFile.index.event, hpfFile.index.indexCount * 40, readCount);
...
BEGIN
  ...
  WHILE HPFSeekToChunk(hpfFile, HPF_SET_CHUNK_ID{ch_data}, chunkPos) DO
    ...
    RawRead(hpfFile.chanId, ADR(readChunk), 16, readCount);
  ...
  i := 0;
  WHILE HPFSeekToChunk(hpfFile, HPF_SET_CHUNK_ID{ch_data}, chunkPos) DO
    RawRead(hpfFile.chanId, ADR(readChunk), 32, readCount);
    RawRead(hpfFile.chanId, ADR(desc), 8, readCount);
...
BEGIN
  ...
  RawRead(hpfFile.chanId, ADR(hpfFile.header), 36, readCount);
  IF (readCount # 36) OR (hpfFile.header.chunkId # ch_header) OR (hpfFile.header.fileVersion # 10001h) THEN
...
VAR
  ...
  dataIdx, valueOffset, readCount : CARDINAL;
  ...
  FOR channel := MIN(HPF_CHANNEL) TO MAX(HPF_CHANNEL) DO
    FOR sensor := MIN(HPF_SENSOR) TO MAX(HPF_SENSOR) DO
      ...
      IF (sensor IN sensors[channel]) AND (timeSampleIdx < maxNumOfSamples) THEN
        ...
        RawRead(hpfFile.chanId, ADR(val), 4, readCount);
